		...
		getFormattedString: function (vCard) {
			...
			if (vCard.email) {
				...
				vCard.email.forEach(
					(address) => {
						if (majorVersion >= 4) {
							formattedVCardString += 'EMAIL' + encodingPrefix + ';type=Email:' + e(address) + nl();
						} else if (majorVersion >= 3 && majorVersion < 4) {
							formattedVCardString += 'EMAIL' + encodingPrefix + ';type=Email,INTERNET:' + e(address) + nl();
						} else {
							formattedVCardString += 'EMAIL' + encodingPrefix + ';Email;INTERNET:' + e(address) + nl();
						}
					}
				);
			}
			...
			if (vCard.url) {
				if (Array.isArray(vCard.url)) {
					vCard.url.forEach(({ type, url }) => {
						formattedVCardString += `URL;type=${type}${encodingPrefix}:${e(url)}${nl()}`
						//formattedVCardString += 'URL' + encodingPrefix + ':' + e(url) + nl();
					});
					//formattedVCardString += vCard.url.map((url) => 'URL' + encodingPrefix + ':' + e(url) + nl())
				} else {
